...
export const VisualCharts: React.FC<VisualChartsProps> = React.memo(({ transactions, currency }) => {
  ...
  const categoryData = useMemo(() => {
    ...
    transactions.forEach((tx) => {
      ...
      if (currency === 'USD') {
        amount = tx.totalUSD ?? (tx.currency === 'KHR' ? tx.amount / (tx.exchangeRate || 4000) : tx.amount);
      } else if (currency === 'KHR') {
        amount = tx.totalKHR ?? (tx.currency === 'USD' ? tx.amount * (tx.exchangeRate || 4000) : tx.amount);
      }
              ...
              <div className="md:col-span-5 flex justify-center relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  ...
                  <span className="text-lg font-bold text-slate-800 font-mono tracking-tight mt-0.5">
                    {hoveredSegment
                      ? formatAmount(selectedData.find((d) => d.id === hoveredSegment)?.value || 0, currency)
                      : formatAmount(selectedSum, currency)}
                  </span>
                  {hoveredSegment && (
                ...
                {trendPathInfo && (
                  <>
                    <text
                      ...
                      textAnchor="end"
                    >
                      {formatAmountNoCents(trendPathInfo.maxBal, currency)}
                    </text>
                    <text
                      x={paddingX - 6}
                      ...
                      textAnchor="end"
                    >
                      {formatAmountNoCents(trendPathInfo.minBal, currency)}
                    </text>
                  </>
                )}
              ...
              <div className="mt-4 flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-200 shadow-2xs select-none">
                ...
                <div className="text-right">
                  ...
                  <span className={`text-sm font-bold font-mono ${
                    ...
                    {formatAmount(hoveredTrendPoint ? hoveredTrendPoint.balance : (trendData[trendData.length - 1]?.balance || 0), currency)}
                  </span>
                </div>
              </div>
            </div>
...
VisualCharts.displayName = 'VisualCharts';
